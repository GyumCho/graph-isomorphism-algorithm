...
class Graph(object):
    ...
    def del_vertex(self, vertex: "Vertex"):
        ...
        if vertex not in self._v:
            raise GraphError("The vertex must be in the graph")

        for i in vertex.incidence:
            self.del_edge(i)
    ...
    def del_edge(self, edge: "Edge"):
        ...
        if edge not in self._e:
            raise GraphError("The edge must be in the graph")
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        if self.directed != other.directed or self.simple != other.simple:
            raise GraphError("Properties of the graphs are not the same")

        old = self.vertices + other.vertices
        graph = Graph(self.directed, len(old), self.simple)
        new = graph.vertices
        mapping = dict(zip(old, new))

        for v in range(len(old)):
            new[v].label = old[v].label
            if hasattr(old[v], 'colornum'):
                new[v].colornum = old[v].colornum

        for i in self.edges + other.edges:
            graph.add_edge(Edge(mapping[i.tail], mapping[i.head]))
