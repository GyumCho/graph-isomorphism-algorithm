...
import sys
from typing import IO, Tuple, List, Union, overload, Literal
...
@overload
def get_graph(file: str, multi: Literal[True]) -> list[Graph]: ...
@overload
def get_graph(file: str, multi: Literal[False]) -> Graph: ...


def get_graph(file: str, multi: bool):
    """
    Returns a graph or a list of graphs given its path.
    :param file: Path of the graph
    :param multi: True to read a list of graphs
    :return: Graph object or list of graph objects
    """
    with open(file) as x:
        g = load_graph(x, read_list=multi)
    return g[0] if multi else g
